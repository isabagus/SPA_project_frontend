...
export default function MentorMasterRubricPage() {
  ...
  const { data: subjects = [] } = useQuery({
    queryKey: ['mentor-subjects'],
    queryFn: async () => {
      const response = await api.get('/mentor/subjects');
  ...
  const { data: rubrics = [], isLoading: isFetchingRubrics } = useQuery({
    queryKey: ['mentor-rubric-master', activeSubjectId],
    queryFn: async () => {
      if (!activeSubjectId) return [];
      const response = await api.get(`/mentor/subjects/${activeSubjectId}/rubrics`);
  ...
  const addCategory = useMutation({
    mutationFn: (name: string) => api.post(`/mentor/subjects/${activeSubjectId}/rubrics`, { rubric_name: name }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] }); setCategoryModalOpen(false); setCatName(""); }
  });

  const updateCategory = useMutation({
    mutationFn: (data: any) => api.put(`/mentor/rubrics/${data.id}`, { rubric_name: data.name }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] }); setCategoryModalOpen(false); setEditingCategory(null); setCatName(""); }
  });

  const deleteCategory = useMutation({
    mutationFn: (id: number) => api.delete(`/mentor/rubrics/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] })
  });

  const addCriteria = useMutation({
    mutationFn: (data: any) => api.post(`/mentor/rubrics/${data.rubricId}/criteria`, { criteria_name: data.name }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] }); setCriteriaModalOpen(false); setCritName(""); }
  });

  const updateCriteria = useMutation({
    mutationFn: (data: any) => api.put(`/mentor/criteria/${data.id}`, { criteria_name: data.name }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] }); setCriteriaModalOpen(false); setEditingCriteria(null); setCritName(""); }
  });

  const deleteCriteria = useMutation({
    mutationFn: (id: number) => api.delete(`/mentor/criteria/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['mentor-rubric-master'] })
  });
    ...
    <div className="p-4 flex flex-col gap-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Assessment Setup (Mentor)</h1>
          <p className="text-gray-500 dark:text-gray-400 font-medium">Configure assessment standards for subjects under your class.</p>
        </div>
        
        <div className="flex flex-wrap gap-3 w-full md:w-auto">
          <div className="flex flex-col gap-1 min-w-[200px]">
            <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Subject</label>
            <select className="w-full text-xs font-bold border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500 transition-all" value={selectedSubjectKey} onChange={e => setSelectedSubjectKey(e.target.value)}>
          ...
          <div className="flex flex-col gap-1 min-w-[120px]">
            ...
            <select className="w-full text-xs font-bold border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500 transition-all" value={selectedTerm} onChange={e => setSelectedTerm(e.target.value)}>
          ...
          <>
            {rubrics.map(rubric => (
              <div key={rubric.rubric_id} className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-xl shadow-gray-200/20 dark:shadow-none overflow-hidden group">
                <div className="px-8 py-6 bg-emerald-50/50 dark:bg-emerald-900/10 flex justify-between items-center border-b border-gray-50 dark:border-gray-700">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-6 bg-emerald-500 rounded-full"></div>
                    <h3 className="font-black text-emerald-900 dark:text-emerald-300 uppercase tracking-widest text-sm">{rubric.rubric_name}</h3>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEditCategory(rubric)} className="p-2 text-gray-400 hover:text-emerald-600 transition-colors">✏️</button>
                    <button 
                ...
                <div className="p-6 space-y-3">
                  {rubric.criteria.map(crit => (
                    <div key={crit.criteria_id} className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl hover:bg-white dark:hover:bg-gray-700 border border-transparent hover:border-emerald-100 dark:hover:border-emerald-900 transition-all group/item shadow-sm hover:shadow-md">
                      <span className="text-sm font-bold text-gray-700 dark:text-gray-300">{crit.criteria_name}</span>
                      <div className="flex gap-2 opacity-0 group-hover/item:opacity-100 transition-opacity">
                        <button onClick={() => openEditCriteria(crit)} className="text-xs font-bold text-gray-400 hover:text-emerald-600 transition-colors">Edit</button>
                        <button 
                  ...
                  <button 
                    ...
                    className="w-full py-4 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl text-xs font-black text-gray-400 hover:border-emerald-400 hover:text-emerald-600 transition-all flex items-center justify-center gap-2"
                  >
            ...
            <button 
              ...
              className="bg-gray-100/50 dark:bg-gray-900/20 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-3xl p-12 text-center group hover:border-emerald-400 transition-all flex flex-col items-center justify-center gap-4"
            >
              <div className="w-12 h-12 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-md group-hover:bg-emerald-600 group-hover:text-white transition-all">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md p-8 shadow-2xl animate-in fade-in zoom-in duration-300 border border-gray-100 dark:border-gray-700">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  type="text"
                  className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500 text-gray-800 dark:text-white transition-all"
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 shadow-lg shadow-emerald-200 dark:shadow-none transition-all active:scale-95 disabled:opacity-50"
                >
